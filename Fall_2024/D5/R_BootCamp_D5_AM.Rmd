...
\
R shiny is a package for program R that allows you to build applications with an HTML front end, and an R backend. That means that the portion of the app that a user sees is built using bits of HTML code pre-packaged into R functions that you can use to build user interfaces (UI). The back end of a shiny app, or server, operates using R code. It is a super useful tool that allows you to bring complex data processing or other complex operations to users without them needing to understand whats happening. 
...
Shiny apps will all have these same 3 parts, the **Global**, **UI**, and **Server**. Some apps may have a lot more going on around these, but all will have these three basic parts. 
